...
class BIN_Interaction_Flat(nn.Sequential):
    ...
    def forward(self, d, p, d_mask, p_mask):
        ...
        i_v = i.view(int(self.batch_size/self.gpus) if torch.cuda.is_available() else self.batch_size, -1, self.max_d, self.max_p) 
        ...
        f = f.view(int(self.batch_size/self.gpus) if torch.cuda.is_available() else self.batch_size, -1)
        #print(f.shape)
