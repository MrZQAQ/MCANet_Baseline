...
from Radam import *
from lookahead2 import Lookahead
...
class Trainer(object):
    ...
    def train(self, dataloader, device):
        ...
        for i,data in trian_pbar:
            atoms, adjs, proteins, labels =[],[],[],[]
            for item in data:
                atom, adj, protein, label = item
                atoms.append(atom)
                adjs.append(adj)
                proteins.append(protein)
                labels.append(label)
            self.optimizer.zero_grad()
            loss = self.model(pack(atoms, adjs, proteins, labels, device))
            loss.backward()
            self.optimizer.step()                
...
class Valer(object):
    ...
    def test(self, dataloader):
        ...
        with torch.no_grad():
            ...
            for i,data in val_pbar:
                ...
                for item in data:
                    ...
                    proteins.append(protein)
                    labels.append(label)
                correct_labels, predicted_labels, predicted_scores = self.model(pack(atoms,adjs,proteins, labels, self.model.device), train=False)
        ...
        return Accuracy, Precision, Reacll, AUC, PRC

    def save_Acc(self, Acc, filename):
