...
def label_by_th(y_pred, threshold=0.5):
    ...
    return y_pred_copy
if no_th:
   evaluation_df = pd.DataFrame(index=["Sen", "Spe", "Pre", "Rec", "Acc", "F1","AUC", "AUPR"])
else:
   evaluation_df = pd.DataFrame(index=["Sen", "Spe", "Pre", "Rec", "Acc", "F1"])
...
for dataset in test_names:
    ...
    print("\tPrecision : ", pre)
    print("\tF1 : ", f1)
    result_dic = {"Acc": acc, "Sen" : sen, "Pre":pre, "F1":f1, "Spe":spe, "Rec":rec}
    if no_th:
